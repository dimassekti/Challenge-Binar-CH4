        ...
        if(et_username.text.isNullOrEmpty()
            ...
            if(et_password.text.toString() != et_password_confirm.text.toString()){
                ...
                GlobalScope.async {
                    ...
                    val result = userDb?.UserDao()?.insertUser(User(null, username, email, password ))

                    activity?.runOnUiThread{
                        if(result != 0.toLong()){
                            Toast.makeText(requireContext(), "Register Berhasil", Toast.LENGTH_LONG).show()
                            view?.let { Navigation.findNavController(it).navigate(R.id.action_registerFragment_to_loginFragment2) }
                        }else{
